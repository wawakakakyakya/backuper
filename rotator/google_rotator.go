...
//ロガーは親で持つので持たせない
// Todo: Remove Logger
type googleRotator struct {
	*baseRotator
	config *config.Config
	logger logger.LoggerInterface
}

func (l *googleRotator) Run() error {
	l.logger.Info("act runner called")
	l._run(l) // input interface
	return nil
}

func (l *googleRotator) _remove(fpath string) error {
...
func (l *googleRotator) _find() ([]string, error) {
...
func newGoogleRotator(config *config.Config, logger logger.LoggerInterface) rotatorInterface {
	br := newBaseRotator(config, logger)
	return &googleRotator{baseRotator: br, config: config, logger: logger}
}
